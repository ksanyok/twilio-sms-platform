...
export default function AppLayout({ children }: { children?: React.ReactNode }) {
  ...
  const commandItems = filteredNav.filter(item =>
      ...
      {!collapsed && (
        <div className="px-3 pt-3">
          <button
            ...
            style={{ backgroundColor: 'var(--bg-input)', color: 'var(--text-muted)', border: '1px solid var(--border-subtle)' }}
          >
            <Search className="w-4 h-4 shrink-0" />
            <span className="flex-1 text-left">Quick navigation...</span>
            <kbd className="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-mono" style={{ backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-faint)' }}>
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <main className="flex-1 overflow-auto">
        {/* Mobile top bar */}
        <div className="sticky top-0 z-30 flex items-center gap-3 px-4 h-14 border-b lg:hidden" style={{
          backgroundColor: 'var(--bg-secondary)',
          borderColor: 'var(--border-subtle)',
        }}>
          ...
          <div className="flex items-center gap-2 flex-1">
            ...
            <span className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>SCL</span>
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[20vh]">
          ...
          <div className="relative w-full max-w-lg mx-4 rounded-xl shadow-2xl overflow-hidden" style={{
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
              ...
              />
              <kbd className="px-1.5 py-0.5 rounded text-[10px] font-mono" style={{ backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-faint)' }}>ESC</kbd>
            </div>
            <div className="max-h-[300px] overflow-y-auto py-2">
              {commandItems.length === 0 ? (
                <p className="px-4 py-6 text-sm text-center" style={{ color: 'var(--text-muted)' }}>No results found</p>
              ) : (
                commandItems.map((item) => (
                  ...
                  >
                    ...
                    <span>{item.name}</span>
                    {item.name === 'Inbox' && unreadCount > 0 && (
                      <span className="ml-auto bg-scl-500 text-white text-[10px] font-bold rounded-full px-1.5 py-0.5">{unreadCount}</span>
                    )}
                  </button>
                ))
              )}
